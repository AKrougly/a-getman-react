...
function findItemByName (items, itemName) {
	...
	if (itemByName.length === 0) {
		...
		throw new Error('Don\'t found item with name:' + itemName.value);
	}
	if (itemByName.length > 1) {
		console.log('У элементов одинаковое имя:' + itemName.value);
		throw new Error('Elements have the same name:' + itemName.value);
	}
	return itemByName[0];
}
...
function scanFields (fields, str, pos = 0) {
	let ind = pos;
	while (str.charAt(ind).match(/[-a-z_0-9.'"[\]]+/i)) {
		ind++;
	}
...
function substParam (str, item, items) {
	...
	while ((objFields.value.charAt(pos) === '.') ||
		...
		if (objFields.value.charAt(pos) === '.') {
			...
			console.log('objFieldName:'+objFieldName.value);
		...
		if (pos < objFields.value.length && typeof objFieldValue === 'object' && objFieldValue !== null) {
			...
			console.log('objFieldValue:'+objFieldValue.value);
...
export default function substParams (item, items, pos = 0) {
	let url = item.url;
	let indOpenBrace = url.indexOf('{', pos);
	while (indOpenBrace !== -1) {
		url = substNestedParams (url, item, items, pos);
		indOpenBrace = url.indexOf('{', pos);
	}
