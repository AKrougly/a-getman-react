...
import { HttpMethods, SendStatuses, } from '../../../stores/consts';
...
export default function ToDoItem({item,  onChange, onSend }) {
  const { uid, name, url, response, error, completed, deleted, sended } = item;
    ...
    <List>
      <ListItemWithWiderSecondaryAction button>
        ...
        {edit ? (
          ...
          <ListItemText primary={url && url.length > 100 ? url.slice(0, 96) + ' ...' : url} onClick={() => setEdit(true)} />
        )}
